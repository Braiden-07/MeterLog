...
**Status: build-order step 6 of 11 — the domain surface in place** (phases 1-3 of 4): auth + tenancy (step 4), RBAC + admin membership management (step 5), and now `assets` with its two children `asset_events` and `readings` — a cursor-paginated REST surface, a lifecycle transition engine, and tenant isolation proven **end to end over HTTP for a user who legitimately holds both tenants**. 266 tests green in CI.

**What that does not cover.** The domain surface is **three tables, not the brief's five**: `maintenance_records` is the remaining v1.0 child of `assets` and is deferred, not dropped — held on a real open question (it is the first _mutable_ table in a schema where everything is soft-deleted or append-only, so whether it gets a `DELETE` grant at all would be the project's first hard delete). **There is no audit trail yet** — and the lifecycle event log is not one: `asset_events` records what happened to a physical asset, `audit_log` records who changed which record, and neither derives from the other. Seven mutation types now await that retrofit at step 7. Invited users still cannot log in until the set-password flow lands (step 8, deadlined step 10, tracked by a blocking Definition-of-Done checkbox). The API is proven by acceptance tests, not by use: no frontend consumes it, no load test has run. Nothing is deployed. → [what is not yet proven](docs/ISOLATION.md#9-what-this-does-not-prove), kept aligned with the enumerated [open items register](docs/DECISIONS.md).
...
npm install
npm run db:migrate
npm run test              # 266 tests, including every DB suite
npm run dev
```
