...
- **`test/api/set-password.spec.ts`'s login-timing test is load-bearing, and it is the ONLY cover for one specific hazard.** Do not delete it as a flaky timing test, and do not loosen its threshold to quiet it.
  - **Why it cannot be replaced by the column grant.** `password_set_at` is withheld from `meterlog_app`, and RLS returns zero rows in the login path anyway (pre-session, no GUCs), so a direct read produces a 500 rather than an oracle. **But `login_lookup` is `SECURITY DEFINER` and bypasses both** — adding `password_set_at` to its `RETURNS TABLE` is a two-line change that looks reasonable and hands the pending flag to login with nothing in the way. Verified by mutation: the pending branch drops to ~15ms against ~43ms, an excess of 1.09x an argon2 verify. Nothing else in the suite notices.
