...
- Test (DB suites only): `npm run test:db` — catalog RLS coverage, the catalog-driven isolation harness, the membership dual-axis proof, the definer probe, the pre-auth definer functions, the membership-write definer functions and their §7 body-level authorization, the tenant-context interceptor, (step 7a) the audit-capture proofs, and (step 8) the set-password / invite-token negatives
- Test (API acceptance): included in `npm run test` — the step-4 auth suite, the step-5 RBAC suite, the revocation-over-HTTP proof, and the step-8 set-password / invite-uniformity suite, all over real HTTP with real signed cookies through the bound interceptor
...
| `readings`     | step 6 phase 2               | `PROJECT_BRIEF.md` §5 :138 + :147 — implied by omission; see below. **Declared and enforced from step 6 phase 2.**         |
...
- **The login-enumeration hazard has THREE layers, and each covers a route the others cannot. Keep all three.** ADR-006 §7 hazard (ii): anything that lets the login path branch on account state is a remotely observable user-enumeration oracle.
  - **Layer 1 — the column grants (catalog assertions 18–20).** `password_set_at` is withheld from `meterlog_app`, and RLS returns zero rows in the login path anyway (pre-session, no GUCs). A direct read from login produces a 500, not an oracle.
  - **Layer 2 — `login_lookup`'s pinned result signature (catalog assertion 21, ADR-017). THE STRUCTURAL FLOOR, and it is what closed the gap layer 1 cannot reach.** `login_lookup` is `SECURITY DEFINER`: it reads `users` as `meterlog_definer` under its own policy, so neither the grant nor RLS applies to it. Adding `password_set_at` to its `RETURNS TABLE` is a two-line change that touches no grant and reds none of 18–20 — verified by mutation at step 8, the pending branch dropping to ~15ms against ~43ms. Assertion 21 pins the result signature as an **equality**, so any new column on that function's return is red deterministically, on a catalog read, with no timing measurement involved.
  - **Layer 3 — the timing test in `test/api/set-password.spec.ts`. Retained, and NOT redundant.** Assertion 21 sees the **returned columns**; it cannot see the **verify path**. A change that skips or short-circuits an argon2 verify — which is what the step-8 defect actually was, one verify for an unknown email and two for a wrong password — adds no column and passes assertion 21 untouched. That class is only observable by measuring. **So: do not delete this test on the grounds that the signature is now pinned, and do not loosen its threshold to quiet it.** A structural pin and a behavioural measurement failing for different reasons is the whole design.
  - **Its threshold is calibrated, not guessed, and the first version was decoration.** It originally asserted relative bounds (within 4x, spread under 3x) and **passed with a real hazard installed** — an argon2 verify is ~30ms against a ~50ms request, so a skipped verify hides inside any loose ratio. It now measures an argon2 verify in-process at `ARGON2_OPTIONS`, measures this machine's noise floor from two arms running identical code, and requires `spread - noise < argonCost * 0.5`. Rotating the arm order each iteration is also load-bearing: a fixed order gave the last arm a stable ~6ms penalty unrelated to argon2.
  - **It found a real defect on its first honest run.** Login performed ONE argon2 verify for an unknown email and TWO for a wrong password — a ~2x existence oracle present since step 4, contradicting what `ISOLATION.md` §8 claimed. See ADR-016.
