...
- **tenants** — an organisation; the tenancy root. One row per customer. Its key IS the tenant, so its RLS policy is keyed on `id` rather than a `tenant_id`. Columns: see [ADR-004](DECISIONS.md#L131).
- **users** — a person. **Pure identity**: globally unique email, and deliberately **no `tenant_id` and no `role` column** — both live on `memberships`. One row per human, shared across every workspace they belong to. Columns: see [ADR-006](DECISIONS.md#L296).
- **assets** — a tracked physical device. One row per asset, soft-deleted; `status` advances only through the transition engine. Columns: see [ADR-007](DECISIONS.md#L335) and the lifecycle contract in [`ARCHITECTURE.md`](ARCHITECTURE.md) §9.2.
- **asset_events** — the lifecycle log. One row per state transition, **append-only**. Columns: see [ADR-007](DECISIONS.md#L335); the append-only declaration and its enforcing grant live in [`CLAUDE.md`](../CLAUDE.md).
- **readings** — meter readings. One row per reading, **append-only**; a correction is a new row, never an edit. Columns: see [ADR-007](DECISIONS.md#L335) and the append-only declaration in [`CLAUDE.md`](../CLAUDE.md).
- **maintenance_records** — work performed on an asset. One row per visit; the one **mutable** child, soft-delete only. Columns: see [ADR-008](DECISIONS.md#L375).
- **audit_log** — the mutation trail. One row per mutated row per mutation, **append-only** and written only by a `SECURITY DEFINER` trigger. Columns: see [ADR-009](DECISIONS.md#L411) and [ADR-011](DECISIONS.md#L485).
- **memberships** — grants one user one role in one tenant; the join the whole two-axis isolation model rests on, and **where `role` lives**. One row per (user, tenant). Introduced by ADR-006 at step 4 and missing from this list ever since, which is the gap this conversion closes. Listed last rather than beside `users` so the seven rows above keep the line numbers that applied migration comments cite by number. Columns: see [ADR-006](DECISIONS.md#L296).
- **invite_tokens** — invite / set-password mechanics. One row per issued redemption token; SHA-256 at rest so a database read yields no usable credential, single-use, 72h, and superseded on re-read. Columns: see [ADR-016](DECISIONS.md#L737). **Not a core domain entity, and the first row here that is not one** — it holds no fact about a tenant's assets or people, only the mechanism by which an invited person reaches a `memberships` row they already have. **The rule it sets for every table after it: an implementation table lives with the ADR that introduces it, and appears in this list only as a pointer.** That keeps the two properties this section exists for from pulling against each other — the **table set stays complete**, so a reader counting rows here against `information_schema` finds nine and nine, while §5 acquires no mechanics it has no reason to own. It is listed after `memberships` for the same reason `memberships` is listed after `audit_log`: appending is the only insertion point that leaves every line number above it untouched.
