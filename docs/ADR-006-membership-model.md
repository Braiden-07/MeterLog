...
- **OPEN-5 — RESOLVED. Accept the residual for v1.0.** Liveness is enforced in-policy only on the two exclusively-reading paths (re-verify, workspace-list subquery), deliberately absent from the `memberships` row policies because the predicate would block the revoking UPDATE itself. Self-axis readers carry `WHERE deleted_at IS NULL` app-side. A third definer function to close the residual was rejected. Full reasoning in the boxed finding in §3.

  **THE APP-SIDE PREDICATES OVER RLS-PROTECTED READS — THE LIST.** This was "the one documented app-side predicate in the design" until G1 added a second, and it is now kept as an enumeration rather than a count. **A number in prose is what drifted in PR #21**; a list cannot silently go wrong when a third arrives, because adding the third means writing its line.

  | Predicate                                                                           | Where                                                                                                             | Why it is app-side and not in a policy                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
  | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
  | `AND m.deleted_at IS NULL`                                                          | `readWorkspaces` ([auth.service.ts:358](../apps/api/src/auth/auth.service.ts#L358))                               | Liveness cannot live in the `memberships` row policies at all: Postgres applies the SELECT policy to the _new_ row of an `UPDATE … WHERE`, so the predicate would block the revoking UPDATE itself. Verified live. Removing it makes a revoked workspace reappear in the switcher.                                                                                                                                                                                                                                 |
  | `WHERE m.tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid` | `MembershipsService.list` ([memberships.service.ts:199](../apps/api/src/memberships/memberships.service.ts#L199)) | G1 / OPEN-13. The self axis and the tenant axis are both permissive and therefore OR, so a multi-workspace caller sees their own rows from every workspace. It cannot be fixed in RLS: a tenant term in `memberships_self_read` would scope this list correctly **and collapse the workspace switcher in the same stroke**, because `readWorkspaces` rides the self axis precisely to see other tenants. Two readers want different row sets from one policy, so the narrowing goes in the reader that wants less. |

  **THE RULE, so this list stays complete.** Any new app-side predicate over an RLS-protected read is added to this table with its purpose and the reason it cannot live in a policy. And the distinction that makes such a predicate legitimate at all: one that **masks** an RLS result — dropping rows RLS should never have returned — is the anti-pattern, because it hides a policy regression behind a redundant clause; one that **narrows a correct RLS result for one endpoint** is not. Both entries above are narrowings, and both are covered by policy-level tests that the predicates are not in the path of ([membership-isolation.spec.ts](../apps/api/test/db/membership-isolation.spec.ts)), so a policy regression is still caught by something these clauses cannot quiet.
