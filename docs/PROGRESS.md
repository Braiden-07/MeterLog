...
- **The admin user-management slice is UNDERWAY: its BACKEND half is merged (ADR-018)** — the pending split (OPEN-14) and G1 (OPEN-13) are **DONE**, so `GET /users/pending` is a safe metadata read, minting is an explicit `POST /users/pending/:membershipId/token`, and the member list is scoped to the active workspace. The definer surface is now **nine** functions. The invite/admin **UI** is the next PR and carries the producer-side `#token=` acceptance bound into OPEN-14.
