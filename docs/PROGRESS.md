...
**Non-vacuity sweep: 22 mutations, 22 red** — 18 against the database and harness, 4 against the citation guard itself. Two worth naming: `actor_user_id SET NOT NULL` reddens _registration_, which is the clearest statement of why the column is nullable — the alternative does not degrade the trail, it closes the product. And admitting `password_hash` to the `users` allowlist reddens both the structural assertion and the behavioural tests, which is what the pairing was for.
