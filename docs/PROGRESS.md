...
- **The step-7 audit retrofit remains an unresolved forward marker.** The membership mutations now exist and are exactly the ones where role-at-time-of-action matters most, and they still write no `audit_log` row. Step 7 must retrofit them, and that is also where **OPEN-4** is answered.
- **The invited-but-uncredentialled dead-end stands** — a set-password / invite-token flow is a later step, and it will need a distinguishing signal to find pending-invite accounts.
