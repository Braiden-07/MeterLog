...
- **`PROJECT_BRIEF` §11 step 9 (E2E + hardening) is UNDERWAY.** PR 1a — **the cross-site posture** — is merged: **login-CSRF was live and is now closed** by a JSON-only body-parser floor (the vulnerability is recorded with its measurement in ADR-001's amendment, which now names TWO conditions rather than one), and **OPEN-20**'s vestigial credentialed CORS is gone. The request pipeline is now one shared definition (`src/bootstrap.ts`) used by production and every acceptance spec, so a floor cannot exist in one and not the other. Remaining in the slice: **OPEN-15** (`X-Expected-Tenant` enforcement — its Due had already elapsed at the admin slice; being paid late here), **OPEN-16** (login rate limiting), **OPEN-17** (Playwright).
...
### 2026-09-18 — `PROJECT_BRIEF` §11 step 9, PR 1a: the cross-site posture

**Backend only.** First PR of the E2E + hardening slice, merged alone because login-CSRF is the highest-severity item in it.

**A REAL VULNERABILITY WAS FOUND AND CLOSED, and it was found by looking rather than by a test going red.** A cross-site auto-submitting HTML form POSTing `email` + `password` to `/auth/login` returned **200** with `Set-Cookie: meterlog_sid=…; Max-Age=28800; HttpOnly; SameSite=Lax` and a body naming the attacker's account and workspace. The victim's browser was then signed in **as the attacker** — every reading, asset and maintenance record they entered afterwards would land in the attacker's tenant. This is login-CSRF, and the direction is what makes it easy to under-rate: it is not the victim's account that is taken, it is the victim's *work*.

**`SameSite=Lax` could never have stopped it.** Lax governs whether an EXISTING cookie is sent; this attack sends none and needs none — it establishes one, and `Set-Cookie` on a top-level navigation is accepted normally. The recorded posture's single condition, "every GET is safe", was also untouched, because the attack is a POST. **Both halves of the recorded posture were true and the system was still exploitable** — the threat had simply never been analysed.

**The floor: state-changing routes accept only JSON.** `bodyParser: false` at creation plus one `json()`, no `urlencoded()` anywhere. A form-encoded body arrives unparsed, the DTO finds no fields, and the request gets the ordinary 400 in the existing `forbidNonWhitelisted` shape — no bespoke CSRF branch, no new error code, nothing to forget on a future route. The layer is the point: an HTML form can only send encodings that do not preflight, and a cross-site `fetch` with JSON must. Refusing to parse form encodings removes the whole no-preflight path in one place.

**OPEN-20 shipped with it because the two compose.** Dropping credentialed CORS leaves nothing to answer the preflight the JSON floor leaves behind. Neither is sufficient alone: CORS was never in a top-level form POST's path, and the JSON floor would not have stopped a preflighted fetch from an allow-listed origin.

**`configureApp` — the extraction that made the fix provable.** Twelve acceptance specs hand-mirrored `main.ts`'s pipeline, copying the prefix and the `ValidationPipe` and silently inheriting Nest's own defaults for everything else — including the body parsers. A security floor applied in `main.ts` alone would have left every test app on the old parsers, so the suite would have asserted the mitigation works against an app that never received it. The pipeline now has ONE definition that production and the tests share. `route-inventory.spec.ts` deliberately does not use it and must not: it issues no HTTP request and omits the prefix on purpose, because §9's rows are controller-relative.

**Gate discipline.** The live negative was written asserting the property, run FIRST against unmitigated code where it failed exactly as quoted, reported, and only then mitigated. Three mutation probes confirm the assertions discriminate: re-enabling `urlencoded` reds only the attack-rejection assertions; removing `json()` reds only the legitimate-login assertions; restoring `enableCors` reds only the CORS negatives.

**Next in the slice:** PR 1b — OPEN-15 (`X-Expected-Tenant` enforcement, 409 on mismatch). OPEN-15's Due had already elapsed at the admin user-management slice; its row is corrected to say so and remains OPEN.

### 2026-09-18 — The admin user-management slice, UI (ADR-018's consumer)

**Pure `apps/web`.** Zero `apps/api`, no migrations. Written against the endpoints PR #26 merged, which is why it needed no backend change: the contracts were landed first precisely so this slice could be pure frontend.

**The members table — two reads, merged by set-difference.** `['members','list']` (`GET /users`) and `['members','pending']` (`GET /users/pending`), both through `session.tenantQuery` so they carry the `['tenant', tenantId, …]` prefix, cancel-on-reset and the generation guard. `pending ⊆ members`, and a row is "Invited" iff its `membershipId` is in the pending set. **The flag is derived because it cannot be sent**: pending-ness is `users.password_set_at`, withheld from `meterlog_app` by column grant, which is the same fact that forces `list_pending_invites` to be a definer function. `mergeMembers` is a pure function with its own spec rather than inline JSX, because it is the only place that distinction exists.

**Invite → mint is TWO STEPS, and that is forced by ADR-016.** `POST /users` answers `{message}` and nothing else — no `membershipId` — because a body varying in shape between the "created an identity" and "attached to an existing one" branches is an account-existence oracle over global identity. So the link cannot be produced at invite time. The flow is: invite → refetch → the row appears as "Invited" → the admin presses "Copy invite link" when ready → `POST /users/pending/:membershipId/token`. That is the same code path as re-sending, so there is one mint path rather than two.

**The minted token is contained.** It lives in exactly one `useState`, cleared on copy and on dismiss. Not `useQuery` (writes to the tenant cache), not `useMutation` (the MutationCache retains it as `data` until GC), not `localStorage`, not a route param, not a log. This is the client-side counterpart of the server's `Cache-Control: no-store` on that one response, and it is pinned by `minted-token-containment.spec.ts` — **a structural pin over the component source, not a runtime measurement**, because a behavioural version needs a DOM testing library this repo does not depend on. The gap is the same shape as the eviction proof's: mechanism proven, real browser is OPEN-17.

**`FORBIDDEN_ROLE` corrects identity; it does NOT reset.** A mid-session demotion answers 403, and the response is as narrow as it can be — re-read `/auth/me`, touch nothing else — so the admin section unmounts on the next render because it is gated on `activeWorkspace.role`. **It is deliberately NOT in `RESET_CODES`**, and a note there says why: the two reset codes mean "you have no workspace", so every tenant-scoped row is unreadable and all of it goes; a demoted admin still holds the workspace and may still read the member list by decision (ADR-006 §3). Adding it would clear rows the caller is entitled to see and remount the shell around them. Proven behaviourally: the tenant cache survives byte-for-byte and the generation does not move, with a contrast test showing a reset code wiping the same fixture.

**`LAST_ADMIN` messaging is on the caller's own row only.** Not a UX guess — the migration states that a live membership is unique per (user, tenant), so if the target is an admin other than the caller, the caller's own admin row is a second live admin by construction and the tenant cannot be zeroed. Demoting or revoking anyone else can never be the last-admin case, so warning on every admin row would describe a refusal that is structurally unreachable there.

**OPEN-14's producer-side acceptance is satisfied here.** `inviteLinkFor` is the single link builder and `invite-link.spec.ts` asserts `#token=` present, `?token=` absent, and — the generalisation that survives a refactor — the token absent from the entire pre-fragment half of the URL.

**Docs.** The stale comment at `set-password/page.tsx` is corrected: it said `list_pending_invites` mints the token, false since #26. OPEN-19 grows to **three** mechanisms, the new one being a bare `[ADR-NNN](#L…)` citation with no literal and no number in its label, where two of the four checks are structurally inert — and whose blast radius is that one ADR index row displaces ~24 of them at once, silently.

**Next:** `PROJECT_BRIEF` §11 step 9.

### 2026-09-18 — The admin user-management slice, BACKEND (ADR-018): the pending split and G1

**Closes OPEN-14 and OPEN-13, as `apps/api` changes only.** The invite/admin UI is a separate PR on top; nothing in `apps/web` is touched here. This is the backend half of the admin user-management slice, which sits on top of slice 1 of the frontend slice (`PROJECT_BRIEF` §11 step 8).

**OPEN-14 — the pending split.** `GET /users/pending` was a state-changing GET: it called `list_pending_invites`, which superseded every live token and minted a replacement on each read. That broke the condition the ADR-001 amendment attaches to `SameSite=Lax` (Lax is CSRF protection only while every GET is safe), and made a window-focus refetch a denial of service against a link the admin had already emailed — routine rather than hypothetical once a UI exists, which is why it was owed *before* the UI.

- `GET /users/pending` is now **metadata only** — `{membershipId, userId, email, role, invitedAt}` — and writes nothing.
- `POST /users/pending/:membershipId/token` mints exactly one token, supersedes any prior live one, returns **201**, and carries **`Cache-Control: no-store`** — the only route in the API that does, because it is the only response body that is a live credential.
- Two response **types**, not one with optional fields: `PendingInvite` has no token, `MintedInviteToken` adds it. A runtime assertion on the GET body's exact key set backs the type, so a future widening still reds.

**The ninth definer function, and a replacement rather than an orphan.** `mint_invite_token(uuid)` is new in `20260916000000` — required because the pending predicate `users.password_set_at` is column-withheld from `meterlog_app`. **`list_pending_invites` was re-created read-only in the same migration rather than merely left uncalled**: it is `EXECUTE`-able by the app role and mints a token for every pending invite per call, so an uncalled version would have been a live credential-minting primitive with no route, no guard and no test in front of it. The split would otherwise have removed the endpoint and kept the capability. The definer surface therefore grows by **one, not two — eight to nine**.

**OPEN-13 (G1) — the cross-workspace member list.** `memberships_self_read` and `memberships_tenant` are both permissive `FOR SELECT` policies and therefore OR, so a multi-workspace admin saw their own rows from every workspace in the active workspace's member list — and `Member` exposes no `tenantId`, so the foreign row was indistinguishable rather than merely extra. Fixed with a tenant predicate in `MembershipsService.list`. **The doc comment that forbade exactly that predicate was rewritten, not deleted**: an app-side filter that MASKS an RLS result is the anti-pattern; one that NARROWS a correct result for one endpoint is this, and the regression concern is answered by `membership-isolation.spec.ts` testing both axes directly as `meterlog_app`.

**Why service-layer and not RLS — now asserted rather than argued.** A tenant term in `memberships_self_read` would scope the member list correctly and **empty the workspace switcher in the same stroke**, because `readWorkspaces` rides the self axis with no tenant predicate precisely so it can name workspaces the caller is not active in. `an RLS tightening WOULD collapse the workspace list` applies that one-line "fix" inside a rolled-back transaction and shows the list drop from two workspaces to one.

**Guards that red on the way through, both greened correctly.** Catalog assertion 4 reds the moment a definer function exists that is not on the reviewed allowlist — greened by adding `mint_invite_token` to `EXPECTED_DEFINER_FUNCTIONS`. The §9 route-inventory guard reds the moment the POST route exists with no row in ARCHITECTURE §9 — greened by adding the row, never by weakening the guard.

**Docs.** ADR-018 records the whole decision. OPEN-5 now **enumerates** the app-side predicates over RLS-protected reads as a table with the rule that any new one joins it — a list rather than the number "two", because a count in prose is what drifted in PR #21. OPEN-11 is **re-targeted, not closed**: `list_pending_invites` no longer returns a live credential, so the pin it owes is owed at `mint_invite_token` instead. OPEN-13 and OPEN-14 are **DONE**, and the producer-side fragment constraint is bound into OPEN-14 as an acceptance for the UI PR (the generated link matches `#token=` and not `?token=`). ISOLATION §9 records the fragment rationale and its limits.

**Next:** the invite/admin UI PR — the consumer of both endpoints, carrying the `#token=` link-builder acceptance. Note for that PR: the doc comment at `apps/web/app/set-password/page.tsx:16` still says `list_pending_invites` mints the token, which is no longer true after this slice. It was deliberately left alone here because this PR touches no `apps/web` file.
...
The audit module, split into two gates. **7a is the capture mechanism and its database-layer proof. 7b — the read surface, the RBAC negatives, the maintenance-edit capstone and the doc refresh — is next and is not started.** 7a deliberately builds **no read endpoints**.

**Part 0 first: the citation guard, because it gates every doc this phase wrote.**

`scripts/check-doc-citations.mjs` (`npm run docs:check`) walks `docs/*.md` and, for every `](path#Ln)` / `](path#La-Lb)` link, asserts (1) the path resolves, (2) the line numbers are inside the file, and (3) — where the link text carries a quoted string, an identifier-shaped token, or a hyphenated record id like `ADR-009` — **that literal appears in the target within five lines of the anchor**.

Points 1–2 catch only deletions and truncations. **Point 3 is the one that earns line-level precision**, because a citation that slides twenty lines when someone adds an import passes both of the others every time. It is affordable because the docs already cite tests by their `it(...)` name, so it mechanises an assertion that was being made by hand.

Wired into CI as **its own step**, before lint/typecheck/test, so a failure reads as "citation drift" and not as a buried test failure — and so a docs-only PR is not told an anchor slid only after a Postgres service container has booted.

**It found two real drifts, unprompted.** On its very first run: the citation for the **citext case-insensitivity regression guard** — the test CLAUDE.md and ADR-004 both single out as the only thing that can catch the silent-operator-resolution class — pointed six lines past its `it(...)`. And when the hyphenated-id rule was added, an `ADR-006` anchor. Both re-anchored. Phase 4's six **file-level** links were then re-anchored to line level, which is now safe: **140 line-level citations, 42 content-checked**, up from 118/20.

**Four ADRs, recorded before the migration.**

- **ADR-009 — capture: trigger-based, mutation-level, role by lookup.** A `SECURITY DEFINER` trigger on each audited table writes the row; the app role never writes `audit_log`. Actor comes from `app.current_user`, which the interceptor already sets unconditionally — **no new plumbing**. Role is **looked up from `memberships`** at write time, not passed as a GUC: a third `app.current_role` GUC would be indistinguishable to a reviewer from the role-in-a-policy DECISION C rejected, and the lookup gives role-at-time for free because the trigger fires _during_ the mutation. **This collapses OPEN-4 from a deferred retrofit into a property of the mechanism.**
- **ADR-010 — integrity: immutable by grant.** `SELECT` and nothing else for the app role. Hash-chaining deferred, with the reason recorded as _engineering_ — a chain serialises inserts on a single contended head, and `readings` is the highest-cardinality table in the schema — rather than as "we ran out of time". What it does **not** defend against (the migration role, a superuser) is stated rather than left implied.
- **ADR-011 — payload: changed-column diff, redaction enforced by the trigger.** A per-table column **allowlist**, carried as the trigger's argument so it lands in `pg_trigger.tgargs` and is assertable from the catalog. Allowlist not denylist: a denylist fails open the day a column is added.
- **ADR-012 — scope, RBAC, volume.** All mutation types uniformly; read gated to **admin and auditor** (enforcement 7b) because gating to admins collapses the auditor role into a technician who cannot write; `audit_log` accepted as the largest table in the database at ~2× reading volume, with retention/partitioning recorded as a marker rather than left as a step-10 discovery.

**Two counts were wrong and are corrected rather than quietly fixed.** `ISOLATION.md` §9 said "**ten** mutation types" over a list naming **eleven**; `OPEN-6`'s title said "**SEVEN**" over a body listing eight. The _names_ were maintained as the list grew at 3b, 3c and phase 4 — the arithmetic was not. Eleven is the number.

**A read-back item came back as a correction, not a confirmation.** Not all audited mutations sit on tenant-scoped tables: **`users` has no `tenant_id`** (ADR-006 §2 made it pure identity), and `register_tenant` inserts the user _before_ the membership with no tenant context set — so for that one write there is no tenant from either source. `audit_log.tenant_id` is therefore nullable on the same rule as `actor_user_id`, and ADR-009 pins one meaning for all three NULLs: **a pre-authentication bootstrap action**. `row_id` is plain `uuid`; the composite-key worry does not land, because ADR-007's composite key is a _foreign_ key and every table keeps a single-column `id`.

**The migration** (`20260914000000_audit_log_capture`, additive) adds the `audit_action` enum (14 values over the eleven named mutations), the `audit_log` table under FORCE RLS with a `FOR SELECT` app policy and a `FOR INSERT` definer policy, `GRANT SELECT` to the app role and `GRANT INSERT` to the definer, `audit_capture()` (definer, `search_path` pinned, every operator over `pg_catalog` types so ADR-004's operator amendment cannot bite), and six trigger attachments each carrying its redaction allowlist.

**Proofs (17 new tests in `test/db/audit.spec.ts`, plus catalog assertions 16 and 17).** Forge, alter and suppress each refused with `permission denied for table audit_log`, asserted on SQLSTATE **and** message with the row-security message excluded. Role-at-time proven twice — a promotion (where the naive join gives the _wrong_ answer) and a revoke (where it gives _no_ answer). Registration succeeds with capture live and writes the NULL-actor system row. Redaction proven **non-vacuously**: the source row is asserted to carry the real hash _first_. DB-layer tenant isolation with M holding a real live admin membership in both tenants.

**Non-vacuity sweep: 23 mutations, 23 red** — 18 against the database and harness, 5 against the citation guard itself (the fifth was added after the guard produced a false negative on its own docs; see ISOLATION §10). Two worth naming: `actor_user_id SET NOT NULL` reddens _registration_, which is the clearest statement of why the column is nullable — the alternative does not degrade the trail, it closes the product. And admitting `password_hash` to the `users` allowlist reddens both the structural assertion and the behavioural tests, which is what the pairing was for.

**One defect found by building, recorded rather than quietly fixed.** The migration's ownership block granted the migration role membership in `meterlog_definer` and **never revoked it** — the standing deployment risk at the top of this file, reintroduced verbatim. **No test caught it and none can:** locally and in CI the migration role is the bootstrap superuser, so the grant branch never runs and the defect is invisible in both environments where the suite executes. It surfaced only from reading the step-4 and step-5 migrations to check the pattern matched. Fixed; `pg_auth_members` verified empty after a from-scratch rebuild.

**Two guards fired on their own, and both were the system working.** `EXPECTED_DEFINER_FUNCTIONS` had to grow to six in the same PR (assertion 4). And `route-inventory.spec.ts` went red the moment `audit_log` joined `APPEND_ONLY_TABLES` — a file that had _predicted_ this in a step-6 comment. Extending it exposed a real boundary: its pairing check demands every append-only resource expose a `GET` **and** a `POST`, which was right while the API was the writer of every such table. For `audit_log` a `POST` is **forbidden**, so the check was **split by writer rather than loosened**.

Assertion 12 was narrowed to exclude trigger-returning functions — Postgres checks `EXECUTE` at `CREATE TRIGGER` time, so granting it would let the assertion be satisfied by an empty gesture — and **new assertion 16 replaces the cover**: a trigger-returning definer function must be _attached_. New assertion 17 asserts no allowlist admits `password_hash` and the trigger is attached exactly where ADR-012 says.

**OPEN register:** OPEN-4 **DONE** (a property of the mechanism, no `app.current_role` GUC introduced). OPEN-6 **DONE** (all eleven captured). **OPEN-7 stays scheduled for step 8** — untouched except for two forward markers added while the reasoning was fresh: (i) it entails a schema change to the frozen `users` table, so it is not a pure feature slice; (ii) **the login path must not branch on that column** — an `if (invitePending) return early` reintroduces exactly the timing class the sentinel-hash construction eliminated, and it is remotely observable. OPEN-9 (hard delete) is now unblocked and remains a separate step.
...
**`readings` is append-only BY CONSTRUCTION — the dangerous kind, foregrounded at the table.** `asset_events` (:137) and `audit_log` (:140) are marked append-only in the brief in words. **`readings` (:138) is not.** It is append-only only because :138 gives it no `updated_at`/`deleted_at` and :147 turns that absence into the property. The migration states plainly that **the authority is the CLAUDE.md declaration, not the absent columns**, and that adding `updated_at` "for consistency with assets" would silently end the property — caught only by catalog assertion 13, which binds the declaration to the grant. This is the entry the whole declaration mechanism was built for.
...
`PROJECT_BRIEF` :151 / :280 require proving one index decision with before/after plans. `readings` is its natural table. 100,000 readings across 5 assets, measured **as `meterlog_app` with the tenant GUC set** so the RLS predicate is part of the plan rather than something measured around:
...
## `PROJECT_BRIEF` §11 step 10 — the deploy-config PR (deploy prep, and it is NOT v1.0)

**What this slice is.** Configuration that prepares a deploy and performs none of it. It provisions nothing, touches no platform account, and ships **no tickable Definition-of-Done box** — every one of them is gated on an author action or on a later slice. Observability is deliberately not here: it carries a new dependency (Sentry) and gets its own reviewed slice.

**The env-surface repair is the load-bearing change, and it is a two-sided defect.** `.env.example` documented `NEXT_PUBLIC_API_URL`, read by **no code anywhere in the repo** — confirmed by `git grep` across every tracked file before deleting it — while `API_ORIGIN`, the variable the entire cross-origin cookie posture depends on, appeared in `next.config.mjs` and **nowhere else**: not in `.env.example`, not in `.env`, not in CI, not in the docs. The one variable that must be right on Vercel was the one the repo never mentioned, and the only variable it did mention for the web app was one whose name invites pointing the browser straight at the API — which is precisely the direct-to-Render call that makes the session cookie cross-site and breaks login.

**That is the `CORS_ORIGIN` dead-variable shape, and it is worse than `CORS_ORIGIN` was.** An inert `CORS_ORIGIN` was harmless because acting on it did nothing. Acting on `NEXT_PUBLIC_API_URL` — wiring it up as its name suggests — **reintroduces the failure**. So it is deleted from both `.env.example` and the local `.env` rather than left flagged, and `API_ORIGIN` is documented in its place with four rules and, for each, the failure it prevents. The first rule is the one that would have cost a deploy cycle to find: `rewrites()` is evaluated when Next loads its config and `next build` bakes the result into the routing manifest, so setting `API_ORIGIN` as a runtime-only variable on Vercel ships the `localhost:3001` fallback and every API call fails against an origin that does not exist — with nothing in the repo wrong and nothing in the build log to read.

**`render.yaml` is a declaration, not an action.** It is inert until an author applies it, and two lines in it are decisions rather than settings. `autoDeploy: false`, because platform git-integration watches the **branch, not the run** — it cannot see whether `verify` and `e2e` passed, so it would ship a red-e2e commit and make DoD :270 tickable while the property it describes was false. And `healthCheckPath: /api/v1/health`, which §16.1 makes part of a security control rather than a monitor: `SessionService` throws at construction without a secret, and a fail-closed guard is only worth what the moment it first runs is worth. The build command carries `npm ci --include=dev` for a reason found by reading rather than by deploying — `NODE_ENV=production` is set on the service, npm omits devDependencies when it sees that, and `@nestjs/cli` and `prisma` are both devDependencies.

**Swagger is public on purpose, and now says so where a reflex would red.** `/api/v1/docs` is served unauthenticated in production. That was true before this PR and was recorded nowhere, which is the actual defect: a deliberate exposure that nothing asserts is one refactor from being an accidental one, in either direction. The mount moved out of `main.ts` into `bootstrap.ts` as `mountOpenApi` — **not** into `configureApp`, which would pay a controller scan on every acceptance spec — so that the spec asserting the exposure calls **production's mount** instead of standing up its own and proving that `SwaggerModule.setup` works. The reasoning for why it is safe is recorded with it: the repository is public and the document is generated from decorators anyone can already read; every endpoint behind it is authn-gated and the exempt set is asserted; and "Try it out" from an anonymous visitor collects 401s, because they hold no session.

**The version advertised there was `0.1.0` — the step-3 scaffold value, on the first line a visitor reads.** It is now `1.0.0-rc.1`: `rc`, not `1.0.0`, because **step 10 is not v1.0** and advertising the milestone would be a claim this build has not earned. `apps/api/package.json` and the root manifest still say `0.1.0`; reconciling the three is a release decision for step 11, recorded rather than quietly done.

**ADR-019 records the CD shape** — a `deploy` job on push to `main`, `needs: [verify, e2e]`, migrations as a step ahead of it and a smoke test behind it. The trap it names is one this repo has already hit once through a different door: the deploy job **must not** become a required status check, because it runs on push and can never report on a pull request, and a required context that never reports blocks the merge button permanently. The precondition it names is that `e2e` is still required by nothing — `ci.yml` says so in the file — so until the ruleset lists it, a red e2e blocks no merge and :270 cannot honestly tick.

### What the docs:check landing pass found

**The predicted sweep happened, exactly as OPEN-19 said it would.** Adding ADR-019's single index row displaced **25 citations** by +1, across `ARCHITECTURE.md`, `ISOLATION.md` and the author-owned `PROJECT_BRIEF.md`. Every one landed on the blank line above its ADR heading, and **the post-fix checker caught all 25** — rule (2b) from the `docs:check`-script fix, which reds on a blank anchor without needing either a literal or a label. This is the third time this displacement has occurred and the first time the tool caught it before a human did; the two earlier instances went green and were found by a landing pass.

The landing pass was still run rather than the +1 applied mechanically: each ADR heading was opened and confirmed to be the ADR its citation names, and the two body-literal citations were confirmed to land on `admin AND auditor` and `@RequiresRole('admin', 'auditor')` at their new lines. `PROJECT_BRIEF.md`'s ten halves are re-anchored under the author-delegated precedent set when the same thing happened at PR #32 — a displacement caused by this PR, repaired by this PR, with no other edit to an author-owned file.

**One failure was this PR's own, and it is a false-positive shape worth recording.** A new citation whose link text was the literal `http://localhost:3001` failed check 4: the checker reads `:3001` as a **line reference** and compared it to anchor `L9`. The label was not wrong, it was ambiguous — a port and a line number are the same shape. Fixed by giving the link the repo's ordinary label form, carrying a checkable literal and an honest line number. Recorded because the next person to cite a URL containing a port will meet it, and the checker's own guidance — never delete the number from the label — is the wrong advice for this one case.

Citations: **212 → 223** (106 content-checked, 141 label-checked). Green on anchors and labels, with `docs:check:self` green ahead of it.

### DoD: nothing ticked, and what is owed where

- **:270 (CI green e2e; deploy automated on `main`)** — owed to the CD job, and separately to the author adding `e2e` to the ruleset. Not this PR.
- **:271 (deployed to HTTPS; Sentry + uptime + `/health` live)** — owed to the observability slice plus the author's platform actions.
- **:267 (frontend covers all essential journeys)** — **stays open into v1.0.** `apps/web/app` holds login, register, set-password and one shell; asset detail, record-reading, maintenance and the audit view do not exist.
- **:269 (≥70% coverage; 2 e2e journeys)** — half true and therefore not tickable: the two journeys are green, and **coverage has never been measured** — `test:cov` exists and no CI step runs it.
