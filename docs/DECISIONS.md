...
- **Consequences:**
  ...
  - **SUPERSEDED BY THE §5 RECONCILIATION — §5 NO LONGER CARRIES COLUMNS AT ALL.** This ADR originally reconciled the brief's index line (:149), and a follow-up reconciled the `audit_log` column sketch (:140) by rewriting it to match the shipped schema. **That second fix was the wrong shape and is superseded**: restating columns in the brief rebuilds the duplication that produced the drift, and it would have drifted again at the next schema change.

    §5 is now a **table index, not a schema**. Each of the eight tables carries its purpose and grain plus a pointer to its governing decision; `audit_log` points here and to ADR-009/ADR-011, and the column list lives in one place — the migration, described by the ADRs. The brief's own authority statement draws the line explicitly: it owns the table set, not the columns.

    **What that changes about this ADR:** the naming decision stands unaltered — `table_name`/`row_id` are the shipped names and the brief's index rule (:149) names them. What is retired is the idea that the brief should carry a column list to be kept accurate. It should carry a pointer, which cannot go stale.

    **And the gap is still one nothing automated sees.** `docs:check` validates citation ANCHORS; nothing validates a document that names a column which does not exist. Pointers are the structural fix for that, because a pointer has nothing to misstate.
