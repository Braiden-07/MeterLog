...
| 016 | Invite-token / set-password flow, and its two floors              | Accepted |
| 017 | `login_lookup`'s result signature is pinned                       | Accepted |
...
| OPEN-10 | **Invite timing distinguishes create-identity from attach-membership.** `POST /users` returns a byte-identical body and status on both branches (ADR-016 took `user_created` off the wire because it was an account-existence oracle over GLOBAL identity), but creating a `users` row plus a membership is measurably more work than inserting a membership alone, so the two branches are still distinguishable to a stopwatch. **Accepted for v1.0, not unnoticed:** the caller is an authenticated, rate-limited admin, and what leaks is existence-ANYWHERE rather than anything tenant-scoped. <br><br>**Enrolled here at the post-step-8 reconciliation.** It was recorded in two places — ADR-016 and the step-8 PROGRESS entry — and in neither of them was it findable by a reader asking "what does this build still owe?", which is the question this table answers. | ADR-016 (step 8, first slice) | **stretch scope** — the BullMQ mailer erases it for free once invite becomes fire-and-return, because the work then happens after the response | OPEN — accepted residual for v1.0 |
| OPEN-11 | **Generalize the result-signature pin to EVERY `SECURITY DEFINER` function.** Catalog assertion 21 pins `login_lookup` alone, because that is where the OPEN-7 hazard was. Every definer function is an enumerated hole in the isolation boundary and every one of them returns data across it, so the end state is that each result signature is pinned. <br><br>**This needs designing, not pattern-matching.** `audit_capture` returns `trigger` and is never called by name — its shape already forced assertion 12 to be narrowed and assertion 16 to be written as the replacement cover. Four of the eight return no table at all (`trigger`, `void`, `void`, `uuid`), where an equality over `pg_get_function_result` is well-defined but buys little, so the value has to be argued per function. And `list_pending_invites` returns a LIVE CREDENTIAL (`token text`) across the boundary, which makes it the function where a pin has the most to say. | ADR-017 forward marker (step 8 close-out) | whichever future phase next touches the definer set | OPEN |
| OPEN-12 | **`actions/checkout` and `actions/setup-node` pinned to a deprecated major in `ci.yml`.** Both were annotating every run. Holding a deprecated action is not a bug until the day it is a broken pipeline, and the cost of moving rises with the number of phases that assume the current behaviour. | step 8 close-out, logged for step 9 | the next docs / CI hygiene pass | **DONE — bumped to `@v5` in the post-step-8 docs reconciliation, the same PR that enrolled this row.** Enrolled as closed rather than left out: the register records that a debt was real, and a debt paid in the same breath it was written down is still evidence the marker was needed — it had been sitting in one line of `PROGRESS.md` and nowhere else. |
...
- **Context:** Step 6 introduces the first tables that are **children of another tenant-scoped table**: `asset_events`, `readings` and `maintenance_records` all belong to an `asset`, and the asset belongs to a tenant. That creates a question the identity/tenancy tables never raised — how does a child row's tenancy stay consistent with its parent's?
  ...
  **(a) is not a free choice — it is what the authoritative documents already specify.** `PROJECT_BRIEF.md` §5 gave `tenant_id (fk)` to `asset_events` (:137), `readings` (:138) and `maintenance_records` (:139) individually when this ADR was written; those per-table column lists were replaced by pointers to the governing ADRs at the §5 reconciliation, so what the brief still states directly is its first design rule (:145), which is "every tenant-scoped table has `tenant_id` and an RLS policy keyed to the current tenant". ADR-006 §3 lists all three among the tables whose policy is "keyed on `app.current_tenant`". This ADR does not re-decide that; it decides the part both documents leave open — **what keeps the denormalized `tenant_id` honest.**
...
- **Consequences:**
  ...
  - **`UNIQUE (id, tenant_id)` on `assets` is not an extra artifact.** `PROJECT_BRIEF.md` §5 (:149) already calls for "composite `(tenant_id, id)` patterns". The unique index is required by Postgres as the FK's referenced target, and it serves the brief's index rule at the same time.
...
- **Consequences:**
  ...
  - **`deleted_at` is an addition to the brief's column list, recorded as such rather than slipped in.** §5 (:139) does not give `maintenance_records` a `deleted_at`, and §5's soft-delete rule (:148) names only `tenants`, `users` and `assets`. Taken literally, a record created in error could never be removed by any means. Soft delete is the **smaller** deviation: one nullable column, every row kept — versus adding a destructive capability the brief also does not grant.
...
- **Decision — RBAC: the trail is readable by admin AND auditor, not admin-only.** Enforcement is step 7b; the decision is recorded here so the read surface is built to it.
  ...
  `PROJECT_BRIEF` §12 (:264) names the trail "viewable by **admin/auditor**", §6 (:167) scopes `GET /audit` "**(admin/auditor)**", and §1 (:14) describes auditors as "read-only + audit access". **Gate the trail to admins and the auditor role collapses into a technician who cannot write** — a role with no capability of its own, in an RBAC matrix whose whole purpose is that the three roles differ. The brief names three roles (:28) and this is the one that distinguishes the third.
...
- **Date:** 2026-09-12
- **Status:** Accepted
- **Context:** ADR-012 recorded that the trail is readable by **admin and auditor** and deliberately left enforcement to this phase. 7a built capture and proved it at the database layer; nothing reads the trail. `PROJECT_BRIEF` §6 (:167) specifies `GET /audit` "(admin/auditor)" with "filter by entity, actor, date range, paginated", and §12 (:264) makes "viewable by admin/auditor" a Definition-of-Done item.
...
- **Decision — pagination: keyset on `(created_at DESC, id DESC)`.** No new DDL: the supporting index `audit_log_tenant_id_created_at_idx` **already exists**, shipped by 7a's migration with `id` deliberately included. Postgres scans a btree backwards, so the ascending index serves the descending order without a second index. **7b therefore adds no migration at all** — an empty migration file would be worse than none.
  ...
  - **Not `id` alone.** `PROJECT_BRIEF` :146 pins UUID primary keys throughout, and a v4 UUID carries no time ordering, so `id` cannot carry the sort. It can only ever be the tiebreaker.
...
- **Consequences:**
  ...
  - The index itself already exists — `audit_log_table_name_row_id_idx`, shipped in 7a and carrying a comment naming :149 as its origin. The brief and the schema now agree.

  - **BOTH :148 AND :140 ARE NOW RECONCILED.** The index line (:148) was reconciled in this phase's first pass. The column sketch (:140) still named `entity_type`, `entity_id`, `before (jsonb)` and `after (jsonb)` — **three** deviations, since ADR-011 also collapsed `before`/`after` into a single `payload jsonb`.

    `PROJECT_BRIEF.md` is **author-owned** (CLAUDE.md, Documentation duties), so :140 was deliberately left standing and flagged here rather than edited quietly; it was reconciled by hand after the 7b merge. It now names the shipped columns, including the three nullables ADR-013 explains and the `no fk` on `tenant_id` — which the original sketch had backwards, since audit and events must never cascade-delete (:149) and a `RESTRICT` foreign key would let the trail **veto the tenant deletion it is recording**.

    **The gap this closes is one nothing automated can see.** `docs:check` validates citation ANCHORS; nothing validates a document that names a column which does not exist. That is why the drift survived a phase, and it is why the manual reconciliation is recorded rather than assumed.

  - **~~The brief's `audit_log` line (:140) now names the shipped columns, and the brief and the schema agree.~~ SUPERSEDED by the §5 reconciliation (2026-09-14) — §5 NO LONGER OWNS COLUMNS AT ALL.** The note above is left standing as the record of why the deviation was logged and how it was first closed; this marks that it has since been closed a second time, structurally.

    Reconciling :140's column list was the right answer to the wrong question. It made the brief **accurate**, and left it **duplicating schema the ADRs own** — so it would have drifted again at the next change to `audit_log`, exactly as it had already drifted once. §5 now carries no columns for any table: each of the eight rows states purpose and grain and points at its governing decision, and `audit_log` reads **columns: see ADR-009 / ADR-011**.

    **What this does and does not change about ADR-015.** The naming decision is untouched — `table_name`/`row_id` are still the shipped names, chosen because a trigger knows a table and a row rather than an entity vocabulary. What is retired is the premise underneath the note: that the brief should carry a column list somebody keeps accurate. It should carry a pointer, which has nothing to misstate.

    **Reading the note above against today's file.** Its `:148` references were correct when written and point at the brief's index rule, which moved to `:149` when §5 gained the `memberships` row. They are deliberately left as written rather than re-anchored: the note is a record of what was true at the time, and this marker is where the present state is recorded. The **live** citation for that rule is in this ADR's consequences, not in the note.

- **Alternatives considered:**
  - **Rename the columns to `entity_type` / `entity_id`** — rejected above: a migration against proven surface, for a worse name.
  - **Leave the brief alone and let the deviation sit in ADR-011 only** — rejected. The brief is the document a reader consults for the data model; leaving it naming columns that do not exist means the next person to implement against :149 writes an index on nothing.

## ADR-016 — The invite-token / set-password flow, and the two floors it had to build first

- **Date:** 2026-09-14
- **Status:** Accepted
- **Discharges:** OPEN-7.
- **Context:** Since step 5, `invite_member` has created an identity whose `password_hash` is a sentinel matching nothing. The invited person could not log in, and could not register their own organisation either — the email was taken (OPEN-1 answers that with a 409). ADR-006 §7 recorded this as a **known, temporary** dead-end and scheduled the fix as the first slice of step 8, with a hard deadline of step 10 (deploy), enforced by a blocking Definition-of-Done checkbox rather than by a comment.

  It left one requirement for this step to inherit: **pending-invite accounts were not distinguishable from credentialled ones by any column.** That was re-verified against a live database before anything was built rather than taken on trust — `register_tenant` and `invite_member` both write an argon2id hash at the identical `ARGON2_OPTIONS` cost (the sentinel is derived from those same parameters by construction), both leave `created_at = updated_at`, both leave `deleted_at` NULL. There is no discriminator.

- **Decision:** `users.password_set_at timestamptz NULL` as the authoritative pending predicate; an `invite_tokens` table holding only SHA-256 hashes; and two `SECURITY DEFINER` functions — `set_password` (redemption, pre-auth) and `list_pending_invites` (tenant-scoped read, minting on read).

- **Consequences:**

  - **`DEFAULT NULL` is a locked invariant, floored by a catalog assertion rather than by a comment.** NULL means pending, so an `invite_member`-created row is pending by default with no change to `invite_member` at all. `DEFAULT now()` — the obvious tidy — would make every invited user look credentialled the instant they were created, `list_pending_invites` would return nothing, no token would ever be minted, and OPEN-7's dead-end would be back **with every test still green**. Catalog assertion 19 pins `column_default IS NULL`, and was proven to red against a simulated `SET DEFAULT now()`.

  - **The backfill is BLANKET `now()`, and blanket is forced rather than chosen.** With no provenance discriminator, no conditional backfill is expressible. The direction is chosen on **asymmetry of error**: blanket NULL would presume every pre-existing account pending, making each one mintable for a set-password token by any admin who can invite their address — an unrecoverable takeover. Blanket `now()` presumes credentialled; the worst case is a genuinely-pending user needing out-of-band admin recovery, an availability failure and a recoverable one. Fail closed toward the recoverable error. It touches zero rows today, and is written as the rule governing the **first real deploy**, not as a no-op.

  - **It must NOT be derived from `audit_log`, and that was checked rather than assumed.** `invite_member` shipped in `20260909000000` and `audit_log` in `20260914000000`, so no invite performed between them has an audit row — an empty audit query reads as "no placeholders found" when it means "cannot see". And, verified live: `audit_capture` logged **both** provenances as `user.created`, so even for rows it could see it did not distinguish them. Population comes from the rows; intent comes from the migration; neither comes from the trail.

  - **`register_tenant` sets `password_set_at = now()` explicitly.** The backfill fixes history; this fixes the future. Without it every founder registered after this migration is born pending, and therefore invite-overwriteable. The column default is **for invite**; registration is the branch that must say the opposite, so it says it in so many words.

### The first floor: the `users` column grants were never actually asserted

`20260907000000` describes the `password_hash` column grant as "asserted in CI"; `20260909000000` repeats the claim. **Both were false, and had been since step 4.** Catalog assertions 6 and 9 are built on `has_table_privilege`, which reports only TABLE-wide privileges and is blind to column grants. Demonstrated against a live database:

- `GRANT SELECT (password_hash) ON public.users TO meterlog_app` — assertion 9 passes, assertion 10 passes, **the whole suite stays green**, and the app role can read every password hash in the system.
- `GRANT UPDATE (password_hash, password_set_at) ON public.users TO meterlog_definer` — assertion 6 returns its expected list unchanged and passes.

So the single most sensitive grant in the schema could have been widened in one line with nothing turning red. **Catalog assertion 18** now pins the `users` column-grant set as an **equality for both roles**, read from `pg_catalog` via `has_column_privilege`. Proven in both directions, and it hardens `password_hash` **retroactively** — the correct blast radius, since the same mechanism was always what protected it.

A second trap was hit while writing it: **`information_schema` is privilege-filtered.** The first version read `information_schema.column_privileges` and `information_schema.columns`, and because the suite connects as `meterlog_app` it could see only the app role's own grants and **could not see `password_set_at` at all** — the assertion would have "passed" over a set it was structurally unable to observe. `pg_catalog` is not filtered; that is why both new assertions use it.

**The grant is a floor under the OBVIOUS path, not under all of them — and the distinction was established by mutation, not assumed.** Two structural defences stop a login-path read of the column: `meterlog_app` holds no grant on it, and RLS returns zero rows anyway, because login runs pre-session with no GUCs set and no `users` policy matches. An `if (invitePending) return early` querying the column directly produces a **500, not an oracle** — verified.

  **But `login_lookup` is `SECURITY DEFINER` and bypasses both.** Extending its `RETURNS TABLE` with `password_set_at` — a two-line change that looks entirely reasonable — hands the pending flag to the login path with no grant and no policy in the way. Verified: with that change plus an early return, the pending branch drops to ~15ms while the others stay at ~43ms, an excess of **1.09x a full argon2 verify** against a measured noise floor of 0.22ms.
...
`ISOLATION.md` §8 claimed: *"The no-such-user login branch performs a real argon2 verify so it costs what the wrong-password branch costs."* It did not. The failure block computed `ok` as a short-circuiting `&&` chain and then ran the dummy verify **unconditionally**:

```
unknown email   -> chain short-circuits, dummy verify       = 1 argon2
soft-deleted    -> chain short-circuits, dummy verify       = 1 argon2
wrong password  -> REAL verify, then ALSO the dummy verify  = 2 argon2
```

A reproducible **~2x split between "this address exists" and "it does not"** — precisely the user-enumeration oracle the dummy verify was introduced to close, sitting inside the construction built to prevent it. Measured across six runs before being diagnosed: the unknown-email branch came in at roughly half the cost of the other two every time.

**Fixed by making the dummy verify the `else` of the real one rather than an addition to it.** Every path now performs exactly one verify and no path performs two.

**The test that found it only worked after being recalibrated, and the first version was decoration.** It originally asserted relative bounds — each median within 4x of the slowest, spread under 3x — and **passed with a deliberate hazard branch installed**: an argon2 verify is ~33ms against a ~50ms request, so skipping it leaves a branch at ~17ms, comfortably inside any loose ratio. The assertion is now stated in the units of the thing being protected: the spread between branches must be under **half of one argon2 verify, measured in-process at `ARGON2_OPTIONS`** rather than hardcoded, so tuning the cost cannot silently invalidate it. That is what surfaced the real defect.

### The rest

  - **SHA-256 at rest, not argon2, and the consequence shapes the read endpoint.** A KDF buys resistance to offline brute force over a low-entropy secret; the token is 244 bits from a CSPRNG, so there is no brute-force surface to buy resistance against and a KDF would only add latency to every mint and redemption. The property actually needed is that a database read — backup, replica, dump — yields no usable credential. **Because the server therefore holds no plaintext it could re-display, `list_pending_invites` MINTS ON READ** and supersedes the prior live token. Re-invite is that same call with no special case.

  - **`user.password_set` was added because `users` UPDATE became reachable for the first time.** The trigger has been attached `AFTER INSERT OR UPDATE` since step 7a, but no role held UPDATE on `users`, so the UPDATE half was dead code that hardcoded `v_action := 'user.created'`. Verified live: without this change the first password set in the system's history would have written an audit row asserting a user was **created**, with an empty diff. The new label is **exhaustive rather than inferred** — the definer's UPDATE grant is column-limited to `(password_hash, password_set_at)` and no other role holds UPDATE, so that is the only reachable `users` UPDATE. Assertion 18 therefore protects the audit label as well as the login floor.

  - **We audit `password_set`, and no admin will ever see it. That is consistent, not broken capture.** `set_password` is pre-session, so the trigger finds no actor and no tenant and writes NULL for both — ADR-013's bootstrap class, which matches no tenant policy and is readable only by direct database access. The payload diff is `{}` (jsonb NOT NULL, never null) because neither written column is on the allowlist, and nothing is lost: WHO is `row_id`, WHAT is the action, WHEN is `created_at`. Stated explicitly so a reader expecting the event in `GET /audit` meets the reason rather than a gap.

  - **`user_created` came off the wire.** It was an **account-existence oracle**: any tenant admin could invite an address, read the flag, and learn whether that person holds an account **anywhere in the system**, including tenants the caller cannot see — `users` is global identity (ADR-006 §2), which is what makes the leak cross-tenant. The flag still exists in the function and still branches token-issue vs membership-only; it simply stops at the service. The invite response is now byte-identical in body and status across both branches.

  - **`invite_tokens` grants the app role NOTHING, which cost assertion 10 its first carve-out.** Every read and write goes through the two definer functions, so a stolen app-role connection cannot enumerate tokens, read a hash, or mint one. Assertion 10 (definer-reachable tables are app-readable) no longer holds for it; the exclusion is **declared** in `APP_UNREADABLE_DEFINER_TABLES`, never derived from live grants, and paired with **assertion 20**, which asserts the app role holds zero privileges — table-level or column-level — on every table in that list. The carve-out asserts the opposite rather than skipping, so a table that became readable fails.

  - **The expiry check sits after the consume for LEGIBILITY, not for a behavioural reason — and this entry records the correction because the first draft claimed otherwise.** A mutation folding `AND expires_at > now()` into the consume predicate produced an observably identical function and **passed the expiry test unchanged**: the RAISE unwinds the consume either way. What actually distinguishes the three token-rejection paths is the table, not the function — forgery matches no row, expiry matches a row with a past `expires_at`, replay matches a row whose `consumed_at` must stay at its first value. The tests assert those discriminators.

  - **`set_password` is exempt from ADR-006 §7 rule (a); `list_pending_invites` is fully bound by it.** The exemption is `register_tenant`'s: pre-authentication, no session, no active tenant, so "is the caller an admin of the active tenant" is undefined rather than skipped. Its authorisation is the token plus the monotonic guard, which permits only the placeholder → real transition and therefore can never overwrite a usable password. `list_pending_invites` acts for an authenticated admin and so checks live-admin-of-active-tenant explicitly and takes the tenant from the GUC, never a parameter.

  - **Accepted residual — invite timing.** Creating an identity plus a membership is measurably more work than a membership insert alone, so the uniform response does not make the two branches indistinguishable to a stopwatch. Accepted for v1.0: the caller is an authenticated, rate-limited admin, and what leaks is existence-anywhere rather than anything tenant-scoped. **The BullMQ mailer in stretch scope erases it for free** once invite becomes fire-and-return.

  - **First migration to acquire `meterlog_definer` membership BEFORE its DDL.** `CREATE OR REPLACE` on a function the definer role already owns requires membership in that role; every prior migration only ever created functions and handed ownership over afterwards. Locally and in CI the migration role is a superuser and the branch is never taken, so a missing grant would be invisible — exactly the class `ISOLATION.md` §9 names. The membership is revoked at the foot, which is the load-bearing half: a migration role left inside `meterlog_definer` silently acquires every `TO meterlog_definer USING (true)` policy, including the new one on a credential table.

- **Alternatives considered:**
  - **argon2 for the token at rest** — rejected above: no brute-force surface to defend, real latency on every mint and redemption.
  - **Returning the token from `POST /users`** — rejected. It would put a credential in the response of an endpoint whose response must be uniform across branches, and it cannot work anyway once the token is hashed at rest and a re-read has nothing to show.
...
- **FORWARD MARKER — generalize the signature pin to all EIGHT definer functions.** Pinning one function's result is the narrow fix for the hazard OPEN-7 opened. The better end state is that **every** `SECURITY DEFINER` function's result signature is pinned, because every one of them is an enumerated hole in the isolation boundary and every one of them returns data across it. That is deliberately **not** folded into OPEN-7's closure: it requires deciding and justifying the expected signature for each of the eight, and `audit_capture` is not merely more of the same — it is a **trigger function**, returning `trigger`, and its shape already forced assertion 12 to be narrowed once and assertion 16 to be written as the replacement cover. A generalized pin needs that case designed, not pattern-matched. **Owed by:** whichever future phase next touches the definer set. Tracked as **OPEN-11**.

  **THE COUNT SAID SEVEN AND WAS WRONG WHEN IT WAS WRITTEN — corrected in place at the post-step-8 reconciliation.** There are **eight**: `login_lookup`, `register_tenant`, `invite_member`, `change_member_role`, `revoke_member`, `audit_capture`, `set_password` and `list_pending_invites`. `EXPECTED_DEFINER_FUNCTIONS` named all eight on the day this marker was written and calls the last two "the seventh and eighth" in so many words. This is a **factual correction, not a superseded decision** — nothing about the marker's reasoning changes — so it is fixed in the sentence rather than marked with a supersession note, which is reserved for a record that a later decision overtook. It is also the third instance of the same drift: `OPEN-6`'s title said SEVEN over a body listing eight, and `ISOLATION.md` §9 said ten over a list of eleven. **The names get maintained as a list grows; the arithmetic does not.**

  **And the omitted function is the one with the most at stake.** `list_pending_invites` returns `token text` — a LIVE REDEMPTION CREDENTIAL — across the isolation boundary, which makes it the definer function where a result-signature pin has the most to say, and it is precisely the one the miscount left out. Four of the eight return no table at all (`trigger`, `void`, `void`, `uuid`), so the generalized pin has at least three shapes to design for, not one.
